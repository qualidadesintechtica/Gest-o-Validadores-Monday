...
6. Abra `Oferta para Produção` e clique em uma aba de período salva.
...
12. Adicione outra regra, alterne entre `E` e `OU` e teste um segundo grupo.
13. Escolha uma coluna oculta, clique em `Aplicar filtros` e confirme o resultado.
14. Use `Limpar todos` e confirme a restauração dos itens.
15. Abra `Colunas`, escolha uma coluna de texto e uma de pessoa e clique em
    `Aplicar colunas`.
16. Localize um item conhecido e anote o valor atual no Monday.
